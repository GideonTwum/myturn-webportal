...
exports.planLegacyWalletRepair = planLegacyWalletRepair;
exports.fetchStaleWallets = fetchStaleWallets;
exports.repairWalletRow = repairWalletRow;
exports.runLegacyWalletRepair = runLegacyWalletRepair;
const client_1 = require("@prisma/client");
const THRESHOLD = new client_1.Prisma.Decimal("0.01");
async function getOrCreateLedgerAccount(tx, accountKey, accountType, userId) {
...
async function postTransferInTx(tx, params) {
    ...
    for (const [accountId, delta] of [
        ...
        const nextBalance = new client_1.Prisma.Decimal(account.balance.toString()).add(delta);
...
function planLegacyWalletRepair(row) {
    const actions = [];
    const balance = new client_1.Prisma.Decimal(row.balance);
    const locked = new client_1.Prisma.Decimal(row.lockedBalance);
...
async function fetchStaleWallets(prisma) {
...
async function repairWalletRow(prisma, row, execute) {
    const balance = new client_1.Prisma.Decimal(row.balance);
    const locked = new client_1.Prisma.Decimal(row.lockedBalance);
    const applied = [];
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (!wallet)
            return;
        const lockedNow = new client_1.Prisma.Decimal(wallet.lockedBalance.toString());
        if (lockedNow.gt(THRESHOLD)) {
            const escrow = await getOrCreateLedgerAccount(tx, `MEMBER_DEPOSIT_ESCROW:${row.userId}:GHS`, client_1.LedgerAccountType.MEMBER_DEPOSIT_ESCROW, row.userId);
            const escrowBal = new client_1.Prisma.Decimal(escrow.balance.toString());
            const gap = lockedNow.sub(escrowBal);
            if (gap.gt(THRESHOLD)) {
                const external = await getOrCreateLedgerAccount(tx, "SYSTEM_EXTERNAL:GHS", client_1.LedgerAccountType.SYSTEM_EXTERNAL);
        ...
        const balNow = new client_1.Prisma.Decimal(refreshed.balance.toString());
        if (balNow.gt(THRESHOLD)) {
            const available = await getOrCreateLedgerAccount(tx, `MEMBER_WALLET_AVAILABLE:${row.userId}:GHS`, client_1.LedgerAccountType.MEMBER_WALLET_AVAILABLE, row.userId);
            const availBal = new client_1.Prisma.Decimal(available.balance.toString());
            ...
            if (gap.gt(THRESHOLD)) {
                ...
                throw new Error(`Refusing repair for ${row.userId}: ledger available exceeds Wallet.balance — manual review required`);
            }
            else {
                applied.push({
                    kind: "zero_balance",
...
async function runLegacyWalletRepair(prisma, execute) {
    ...
    for (const row of rows) {
        console.log(`[repair:legacy-wallet] user=${row.userId} balance=${row.balance} locked=${row.lockedBalance}`);
        const actions = await repairWalletRow(prisma, row, execute);
        for (const action of actions) {
            console.log(`  → ${action.kind}: ${action.reason}${"amount" in action ? ` (${action.amount})` : ""}`);
        }
    }
    if (!execute) {
        ...
        console.log(`[repair:legacy-wallet] execute complete — ${remaining.length} stale row(s) remaining`);
    }
}
//# sourceMappingURL=repair-legacy-wallet-staging.lib.js.map